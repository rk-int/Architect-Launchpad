  ...
  return (
    <div className="relative min-h-screen pb-12">
      ...
      <div className="relative z-10 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <button
                ...
                className={`neo-glass rounded-2xl p-4 text-left neo-glass-hover group cursor-pointer relative overflow-hidden transition-all duration-300 ${
                  showPoster ? 'border-orange-500/30 shadow-[0_0_15px_rgba(255,107,0,0.1)]' : 'border-transparent'
                }`}
              >
                {/* Pulsing Eye-catching Notification Dot when hidden */}
                {!showPoster && (
                ...
                <div className="w-9 h-9 rounded-xl bg-orange-600/15 border border-orange-500/30 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform shadow-[0_0_12px_rgba(255,107,0,0.15)]">
                  <Map size={16} className="text-orange-400" />
                </div>
                <h4 className="text-sm font-bold text-slate-100 flex items-center gap-1.5">
                  {showPoster ? 'Hide Roadmap Poster' : 'Show Roadmap Poster'}
                </h4>
                <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                  {showPoster ? 'Hide inline infographic' : `View ${activeWeeks.length} Weeks infographic`}
                </p>
              </button>
            </div>
          </div>
        ...
        {/* ── INLINE ROADMAP POSTER ── */}
        {showPoster && (
          <div className="neo-glass rounded-3xl p-5 md:p-6 border-white/5 space-y-4 animate-fade-in relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-white/5 pb-2">
              <div className="flex items-center gap-2">
                <Map size={14} className="text-orange-400" />
                <h3 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider">
                  Claude Certified Architect Study Journey
                </h3>
              </div>
              <span className="text-[10px] font-mono text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded border border-orange-500/20">
                {activeWeeks.length} Weeks Plan
              </span>
            </div>
            <div className="flex items-center justify-center bg-slate-950/40 rounded-2xl p-2 border border-white/5">
              <img
                src={`/assets/roadmap-${activeWeeks.length}w.png`}
                alt={`${activeWeeks.length} Weeks Roadmap`}
                className="max-w-full h-auto object-contain rounded-xl max-h-[450px] shadow-2xl"
              />
            </div>
          </div>
        )}
